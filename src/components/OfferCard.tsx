...
const OfferCard: FC<CardCategory3Props> = ({ className = '', taxonomy }) => {
	...
	return (
		...
		>
			...
			<div className="mt-4 truncate">
				...
				<span
					className={`mt-1.5 block text-sm text-neutral-6000 dark:text-neutral-400`}
				>
					{convertNumbThousand(count || 0)} properties
				</span>
			</div>
			<div className="absolute left-3 top-2 flex w-full flex-wrap">
				<p className="rounded-full bg-red-500 px-3 text-[.7rem] text-white">
					First reservation 50% off
				</p>
			</div>
		</Link>
	)
