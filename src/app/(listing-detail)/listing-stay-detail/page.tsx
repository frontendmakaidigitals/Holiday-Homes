  ...
  const HandleReserve = async () => {
    ...
    try {
      const geoResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_IP_GEO_URL}`,
      );

      const trackingData = {
        placeName: listings?.propertyTitle,
        city: geoResponse.data.city?.name, // Use .name from city object
        country: geoResponse.data.country?.name, // Use .name from country object
        state: geoResponse.data.state?.name, // Use .name from state object
